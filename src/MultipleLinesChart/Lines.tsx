import React, { useEffect, useMemo } from 'react';
import {
    select,
    line,
    ScaleBand,
    ScaleLinear,
    ScaleTime,
    curveMonotoneX,
} from 'd3';
// import { LineChartDataItem } from './LineChartBasic';
import { SvgContainerData } from '../SvgContainer/SvgContainer';
import { LineGroupData, LineVertexData } from './types';

type Props = {
    xScale: ScaleTime<number, number> | ScaleLinear<number, number>;
    yScale: ScaleLinear<number, number>;
    svgContainerData?: SvgContainerData;
    data: LineGroupData[];
    stroke?: string;
    width?: number;
};

const Lines: React.FC<Props> = ({
    xScale,
    yScale,
    data,
    svgContainerData,
    width = 1,
}) => {
    const lineGroup = React.useRef<SVGGElement>();

    const valueline = useMemo(() => {
        return line<LineVertexData>()
            .curve(curveMonotoneX)
            .x((d) => xScale(d.x))
            .y((d) => yScale(d.y));
    }, [xScale, yScale]);

    const draw = () => {
        remove();

        select(lineGroup.current)
            .selectAll('.line')
            .data(data)
            .enter()
            .append('path')
            .attr('fill', 'none')
            .attr('stroke', (d) => d.fill)
            .attr('stroke-width', width)
            .style('stroke-dasharray', (d) => {
                return d.dashPattern || 'none';
            })
            .attr('d', function (d) {
                return valueline(d.values);
            });
    };

    const remove = () => {
        const lines = select(lineGroup.current).selectAll('path');

        // check the number of existing lines, if greater than 0; remove all existing ones
        if (lines.size()) {
            lines.remove().exit();
        }
    };

    useEffect(() => {
        if (svgContainerData && xScale && yScale && data) {
            draw();
        }
    }, [xScale, yScale, data, svgContainerData]);

    return <g ref={lineGroup} className="multiple-lines-group"></g>;
};

export default Lines;
